class Preprocessor
  def initialize(input = nil, define_hash = {}, device_hash = {}, register_hash = {})
    ...
    @define_hash[:register_unused] = {:val => 'RsvdP'} if !define_hash.has_key?(:register_unused)
    @define_hash[:register_level]  = {:val => '2'} if !define_hash.has_key?(:register_level)
    @define_hash[:register_width]  = {:val => '32'} if !define_hash.has_key?(:register_width)
    @src_map                       = {}
    @chunks                        = []
    @processed                     = false
